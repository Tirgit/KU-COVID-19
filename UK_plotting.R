...
q <- ggplot(data = res_lonely, aes(x = reorder(Strata, percent), y = percent, fill = barcolor)) +
  ...
  ylab("Proportion with high levels of loneliness") +
...
pdf("UK_stratified_loneliness.pdf", width = 6, height = 3)
q
dev.off()
...
#multichoice plot
p <- ggplot(data = results_worries, aes(x = fct_rev(variable) , y = percent, fill = value)) +
  geom_bar(stat="identity", width = 0.7) +
  coord_flip() +
  ylab("Percentage") + 
  xlab("Worries") +
